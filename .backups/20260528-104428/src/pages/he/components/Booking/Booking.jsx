...
const Booking = forwardRef(function Booking(_, ref) {
  ...
  const pct = useMemo(() => {
    ...
    if (channel) score += 2
    if (hours) score += 2
    if (tableDesign) score += 3

    return Math.min(100, score)
  }, [values, time, channel, hours, occasion, vibe, budget, diet, drinks, tableDesign])
            ...
            <div className={styles.formSection}>
              {!tableDesign ? (
                ...
                <div className={styles.upgradeOpen}>
                  <button type="button" className={styles.upgradeRemove} onClick={() => setTableDesign(false)} aria-label="הסר שדרוג">×</button>
                  <div className={styles.upgradeOpenHead}>
                    <div className={styles.upgradeEyebrow}>שדרוג נבחר · בתשלום</div>
                    <div className={styles.upgradeOpenTitle}>עיצוב שולחן ואווירה יוקרתית</div>
                  </div>
                  <div className={styles.upgradeMedia}>
                    <video
                      className={styles.upgradeVideo}
                      src="/uploads/table-design-1.mp4"
                      poster="/uploads/table-design-1.jpg"
                      autoPlay muted loop playsInline preload="metadata"
                    />
                    <img className={styles.upgradePhoto} src="/uploads/table-design-1.jpg" alt="עיצוב שולחן יוקרתי" />
                  </div>
