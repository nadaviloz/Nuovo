...
export default function MenuTiers({ value, onChange, guests }) {
  ...
  const setPastas = (next) => onChange({ ...value, basics, pastas: next, fishes })
  const setFishes = (next) => onChange({ ...value, basics, pastas, fishes: next })
  ...
  const closePasta = () => { setPastaOpen(false); setPastas([]) }
  const openFish = () => setFishOpen(true)
  const closeFish = () => { setFishOpen(false); setFishes([]) }
  ...
  return (
    <div className={styles.tiers}>
      ...
      {!fishOpen ? (
        ...
        <div className={styles.tierWrap}>
          <button type="button" className={`${styles.remove} ${styles.removeDark}`} onClick={closeFish} aria-label="הסר שכבה">×</button>
          <TierSection
